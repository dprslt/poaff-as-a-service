...
def convertJsonCalalogToCSV(cat:dict) -> str:
    ...
    # Check if headerFile exists, if not create default headers from first item
    if not "headerFile" in cat:
        if len(cat) > 0:
            # Get the first non-header item to determine structure
            sample_item = None
            for key, val in cat.items():
                if isinstance(val, dict) and key != "headerFile":
                    sample_item = val
                    break
            
            if sample_item:
                # Create a default header structure from the first item
                oHeader = {"srcFile": {"srcFileName": "unknown", "srcFileDate": "", "srcFileSize": ""}}
                for key in sample_item.keys():
                    if isinstance(sample_item[key], dict):
                        oHeader[key] = sample_item[key]
                    else:
                        oHeader[key] = {"value": str(sample_item[key])}
            else:
                oHeader = {"srcFile": {"srcFileName": "unknown", "srcFileDate": "", "srcFileSize": ""}}
        else:
            oHeader = {"srcFile": {"srcFileName": "unknown", "srcFileDate": "", "srcFileSize": ""}}
                ...
                if isinstance(val2, dict):
                    for key3,val3 in val2.items():
                        if isinstance(val3, str):
                            val3 = val3.replace(chr(34), chr(39))
                        csvVal += '"{0}";'.format(val3)
                else:
