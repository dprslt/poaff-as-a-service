...
class AixmTools:
    ...
    def make_arc(self, Pcenter, radius, start_angle=0.0, stop_angle=360.0, clockwiseArc=False):
        ...
        # Convert Point object to array of [lat, lon]
        center_coords = np.array([Pcenter.x, Pcenter.y])
        
        # Generate the arc points
        polygon = geog.propagate(center_coords, angles, radius)
        
        # Convert to the required format with proper rounding
        for o in polygon:
