...
class XmlSIA:
    ...
    def openFile(self, sSrcFile:str, sKeyFile:str) -> None:
        ...
        encodings = ['iso-8859-1', 'cp1252', 'utf-8', 'latin1']
        for encoding in encodings:
            try:
                with open(sSrcFile, 'r', encoding=encoding) as f:
                    content = f.read()
                    self.doc = BeautifulSoup(content, "xml", from_encoding=encoding)
                    ...
                    self.oLog.info(f"Successfully parsed file using {encoding} encoding", outConsole=True)
                    return
            except UnicodeDecodeError:
                continue
            except Exception as e:
                self.oLog.error(f"Error parsing with {encoding}: {str(e)}", outConsole=True)
                continue
                
        # If we get here, none of the encodings worked
        raise ValueError("Could not parse the XML file with any of the supported encodings. Please check the file encoding.")
